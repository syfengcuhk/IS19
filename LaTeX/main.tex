...
% One issue is that DPGMM frame clustering requires high computational costs.
...
% with details described in \cite{hsu2017nips}.
...
% for each target zero-resource language, 
...
% applied to perform frame clustering towards speech features. 
...
% Test sets are organized into subsets of differing utterance length (1s, 10s and 120s). 
...
% Dissimilarity metrics are dynamic time warping and cosine distance.
...
% Experiments are implemented in Tensorflow \cite{Abadi2016tensorflow}.
...
% In this setting, $\bm{z_1}$ outperforms MFCC \cite{dunbar2017zero} in both across- and within-speaker conditions.
...
\centering
\caption{ABX error rates ($\%$) on DNN-BNFs trained with/without FHVAE-based speaker-invariant features}
...
% \footnote{Gender information is confirmed through listening.}. 
...
% structure and loss function of DNN-BNFs mentioned here are 
...
% ($17.09\%$, $3$-rd  bar from left in Figure \ref{fig:recon_abx}). 
$\bm{\hat{x}}$-s0107 represents the general case as s0107 performs moderately
% ($17.52\%$, $1$-st bar from left in Figure \ref{fig:recon_abx}) 
...
% (1) The DNN-BNF model trained with  $\{\bm{\tilde{x}}\}$ or $\{\bm{z_1}\}$ outperforms that trained with MFCCs. The baseline system and  systems \X1 \& \X2 differ only in DNN-BNF inputs. The results demonstrate the effectiveness of replacing DNN inputs with more speaker-invariant features in performance improvement.
...
% frame labeling based on speaker-invariant feature representation.
...
% ($17.3\%$ relative) ABX error rate reduction in across-speaker condition, and $0.6\%$ absolute ($7.3\%$ relative) reduction in within-speaker condition, as compared to the baseline DNN-BNF system. 
